...
const nativeImageAiSource = readFileSync(new URL('../../src/js/spatial-native-image-ai.js', import.meta.url), 'utf8');
...
test('Excalidraw is isolated behind a user-triggered dynamic import', () => {
  ...
  assert.match(island, /PRODUCT_CANVAS_BACKGROUNDS = Object\.freeze\(\[[\s\S]*?'#e7e6e3',[\s\S]*?'#222528',[\s\S]*?'#d4d0cb',[\s\S]*?'#cfd3d7'/);
  assert.match(island, /const EXCALIDRAW_DOCUMENT_THEME = 'light'/);
  assert.match(island, /const EXCALIDRAW_DOCUMENT_BACKGROUND = 'transparent'/);
  assert.match(island, /theme=\{EXCALIDRAW_DOCUMENT_THEME\}/);
  assert.doesNotMatch(island, /theme=\{theme\}/);
  assert.match(island, /runtimeSceneForCanvas\(canvasDocument\.scene\)/);
  assert.doesNotMatch(island, /new MutationObserver/);
  assert.match(island, /initialData=\{initialScene\}/);
  assert.match(island, /appState: persistentAppState\(appState\)/);
  assert.match(stableUiCss, /--canvas-ui-background: #e7e6e3/);
  assert.match(stableUiCss, /--canvas-ui-background: #222528/);
  assert.match(stableUiCss, /--canvas-ui-grid: rgba\(33, 36, 40, \.09\)/);
  assert.match(stableUiCss, /--canvas-ui-grid: rgba\(255, 255, 255, \.075\)/);
  assert.match(stableUiCss, /\.spatial-canvas-host \{ color-scheme: light; \}/);
...
test('G4B, Result variation, Inspector, and Ctrl+K share one governed Canvas Native AI task path', () => {
  ...
  assert.match(nativeImageAiSource, /packaging_text: true/);
  assert.match(nativeImageAiSource, /logo: true/);
