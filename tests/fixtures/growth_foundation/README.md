...
JSON Schema enforces the structural boundary. `tests/test_growth_foundation_contract.py` also freezes semantic invariants that JSON Schema cannot express by itself: referenced entities must exist, entity IDs must be unique within each collection, asset layers must use declared source assets, ROI rectangles must stay inside normalized source bounds, and the undo cursor must stay inside operation history.
