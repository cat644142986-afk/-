...
同身份未签名 NSIS 位于 `build/installer-candidate/146e198a2a90a68d425f6252d0f800e96faede0a/Product Atelier_1.0.0_x64-setup.exe`，`110,054,894 bytes`，SHA-256 `5BEE6D420932F900FFE8556DDCF99888C54B1C167A931EEABB85C1045854DBD4`；manifest SHA-256 为 `0FCC6E8B5F3B85768D1218FE94B88D1324BB388E4700C8658774E6811E009505`，预计安装后 App SHA-256 为 `3E74856F9C0321667573365E59D7D91ED0C05AB2B5FE587F80E514DDA7E82178`。隔离安装、安装态 sidecar/App、v5/v7→v8 迁移、视频任务/重启、静默卸载、注册表与快捷方式恢复均通过，卸载后无 Product Atelier 安装项残留。

本次新增视频导入链还用精确打包 sidecar 实测了 WebM 导入、JPEG 代理封面、原视频下载 SHA-256 一致和 sidecar 重启恢复；证据目录为 `D:\ProductAtelier-Temp\ic6-final-video-146e198-20260908`。源码门禁保持前端 `259/259`、Python `541` 项（`537` 通过、`4` 项平台条件跳过）、Vite production、lazy bundle、locked Rust/Tauri、Ruff、语法与 Git whitespace 全绿。未读取 API Key、调用付费接口或写入正式账本。

非阻塞边界：当前原生窗口自动化桥不能录制修复后的真实 Explorer 鼠标拖放手势，因此该入口由前端真实 `FileList` 路由回归与精确打包 sidecar 文件导入闭环共同证明；125% DPI 没有单独新增截图，100%/150% 边界和 scale-aware Tauri 日志已通过。未签名 NSIS 仅为内部候选；公开分发仍需外部代码签名证书。以上不阻塞 IC6 阶段关闭，但不得扩写成公开签名发布或正式便携版提升。

正式保护复核：正式 App、sidecar、manifest 和桌面快捷方式 SHA-256 仍分别为 `F0D1A07313A47258CD17FA4143DF4F0069D6E075CAA9D22B00EB7220A15685A7`、`2AF3FC31A90961D77A41BE6CECAFD3559E0D0FFFB505FD9BBEA9EC7C61293894`、`287042C051B97CC9F60C42F670C470EBEC965042378B790E0F4F13FCC9718A38`、`B74D4853F471A0A9B5E56C1CF6CFD34C9E66CD1D4393144CA03E4479FD9A4BEC`；快捷方式目标和工作目录未变。正式账本当前 SHA-256 为 `9D659E2D6453CE426D5DD2DC7CC4AC3C4011566CDA6EB0CC9C9058C3A5C70305`，最后写入早于本轮封板测试；本轮所有运行均使用隔离数据目录。正式版本继续保持 `93539f0 / contract .3 / schema v7`。
