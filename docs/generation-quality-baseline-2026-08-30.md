...
## Prompt v1/v3 付费单变量执行（2026-09-01，待用户盲评）
...
本地确定性门禁为 5 张通过、1 张阻断失败；失败轴是非白边界和严重画布接触。其盲评身份暂不公开，产品数量、包装文字、参考图保真和真实边缘质量仍必须人工判断。A/B 联系表、私有映射、六张成品和第一轮 AI 盲评分数保留在 Git 忽略目录；第一轮评分已在未读取映射时封存。结构化执行证据位于 `docs/reports/generation-prompt-paid-ab-2026-09-01.json`。

当前只完成“执行”，质量结论仍为待用户第二轮盲选。用户完成包装、数量、透明瓶三行的 A/B 选择后才允许揭盲和决定；此前 `prompt_v3` 继续默认关闭，不提升正式便携版，也没有新的付费调用授权。
