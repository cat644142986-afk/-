...
这组数据支持“单次策略值得继续验证”，但不支持立即切换正式默认：样本只有 3 个且只完成 1 轮盲评，尚未达到既定的两轮/9 输入推广门槛。正式桌面软件继续使用 `prompt_v1 + legacy_double_pass`；下一步先做第二轮独立盲评，再决定是否扩到 9 输入。`prompt_v3` 必须另做单变量实验，不能与阶段策略一起切换。完整结构化结果位于 `docs/reports/generation-strategy-paid-ab-2026-08-31.json`。
