...
- 右侧任务控制区在 1280×720 实测无内部滚动；文档页面无横向或纵向溢出。
...
2. 约 960×600 时把右侧任务控制区正式切换为可关闭抽屉，而不是继续压缩主舞台。
3. 在 Tauri 便携壳完成 Windows 100%/125%/150% DPI、最大化、最小尺寸和多显示器验证。
4. 验证窗口 24px CSS 几何与 Windows 11 实际外框在所有 DPI 下无底层漏出。
5. 完成排版可读性审计：核心中文与交互文字不得靠 7–10px 缩小来换取“无滚动”，装饰性英文与核心文案必须分级处理。
6. 补齐空、加载、离线、冲突恢复等状态的统一组件和键盘/焦点回归。

Phase 5 仍需：完整素材管理器、撤销提示、回收站和引用解释 UI；Task Dock 的跨工作流筛选与真实 10+20 并发场景。

## 5. 下一步执行顺序

1. 先完成 Phase 4 的打包壳/DPI/窄屏门禁与模块边界。
